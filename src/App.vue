	...
	<header class="navbar navbar-expand navbar-dark bg-dark fixed-top shadow">
		<div class="container">
			<div class="navbar-brand my-1">
				<i class="bi bi-whatsapp text-secondary h2 align-middle me-3"></i>
				...
				<a class="text-light align-middle" href="https://github.com/italofds/whatsapp-new-ip-processor" target="_blank">whatsapp-new-ip-processor</a>
	...
	<main id="start" style="padding-top: 70px;">
		<div class="container p-3">
			<div class="bg-body-tertiary mb-3 p-3 p-lg-5 rounded">
				<h1 class="mb-3">Processador de dados de IP da plataforma WhatsApp</h1>
				<p class="lead">Ferramenta de processamento para tratar as respostas da empresa <strong>Meta Platforms, Inc.</strong> acerca dos endereços IP solicitados judicialmente, especificamente através da plataforma <strong>WhatsApp Records</strong>.</p>
			...
			<div class="my-5" v-show="resultList.length > 0">
				...
				<div class="tab-content">
					<div class="tab-pane fade show active" id="list-tab-pane" role="tabpanel" aria-labelledby="list-tab" tabindex="0">
						...
						<div v-if="ipQueryArray.length > 300" class="alert alert-danger" role="alert"><strong>Atenção!</strong> Devido a grande quantidade de endereços IP diferentes, o sistema se limitará a consultar os provedores de conexão dos primeiros 300 endereços IP.</div>
			...
			<p class="text-muted small text-center">Desenvolvido por <a target="_blank" href="https://github.com/italofds">Ítalo Santos</a> | © 2024 | Licença GPL-3.0 | Hospedado pelo GitHub Pages | Contribua: <a target="_blank" href="https://github.com/italofds/whatsapp-new-ip-processor">github.com/italofds/whatsapp-new-ip-processor</a></p>	
		</div>
	</main>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h1 class="modal-title fs-5" id="exampleModalLabel">Filtrar Registros</h1>
	...
	methods: {
		...
		convertDatetimeFormat: function (dateValue, format) {
			if (dateValue) {
				...
				const targetTime = momentUtcDate.utcOffset(this.formData.selectedTimezone);
		...
		mountQueryList() {
			for(let resultItem of this.resultList) {
				...
				if(!isRepeated) {
					...
					if(this.ipQueryArray.length <= 300) {
						ipQueryItem.status = "loading";		
						ipQueryItem.isp = "";
					} else {
						ipQueryItem.status = "error";
						ipQueryItem.isp = "CONSULTA EXCEDIDA";
					}
		...
		async fetchData() {
			...
			const queryList = this.ipQueryArray.filter(function (el) {
				return el.status == "loading";
			});
